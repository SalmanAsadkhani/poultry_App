...
class _CycleListScreenState extends State<CycleListScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _refreshCyclesList() async {
    setState(() {
      ...
      _cyclesFuture = DatabaseHelper.instance.getTopLevelCycles();
  ...
  // اگر دوره فعال است، چک داده (خود دوره + تمام سالن‌هایش)
  if (cycle.isActive) {
    // چک داده‌های خود دوره
    bool hasData = await DatabaseHelper.instance.hasRelatedData(cycle.id!);

    // اگر چند سالنه است، داده‌های هر سالن را هم چک کن
    if (!hasData && cycle.isMultiHall) {
      final halls =
          await DatabaseHelper.instance.getHallsForCycle(cycle.id!);
      for (final hall in halls) {
        if (hall.id != null) {
          final hallHasData =
              await DatabaseHelper.instance.hasRelatedData(hall.id!);
          if (hallHasData) {
            hasData = true;
            break; // همین که یک سالن داده داشت کافیه
          }
        }
      }
    }
    ...
    if (hasData) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            cycle.isMultiHall
                ? 'این دوره فعال است و یک یا چند سالن آن دارای گزارش/اطلاعات است و قابل حذف نیست.'
                : 'این دوره فعال دارای گزارش/اطلاعات است و قابل حذف نیست.',
          ),
  ...
  final confirm = await showDialog<bool>(
    ...
    builder: (ctx) => AlertDialog(
      ...
      content: Text(
        cycle.isMultiHall
            ? 'آیا از حذف «${cycle.name}» و تمام سالن‌های آن مطمئن هستید؟'
                      ...
                      return SlideTransition(
                        ...
                        child: Card(
                          ...
                          child: InkWell(
                            ...
                            if (cycle.isMultiHall) {
                              final halls = await DatabaseHelper.instance.getHallsForCycle(cycle.id!);
                              if (!mounted) return;
                              Navigator.push(
                                context,
                                MaterialPageRoute(
                                  builder: (_) => HallSelectionScreen(parent: cycle, halls: halls),
                                ),
                              ).then((_) => _refreshCyclesList());
                            } else {
                              Navigator.push(
                                context,
                                MaterialPageRoute(
                                  builder: (_) => CycleDashboardScreen(cycle: cycle),
                                ),
                              );
                              _refreshCyclesList();
                            }
