...
class _AddDailyReportScreenState extends State<AddDailyReportScreen> {
  ...
  _feedTypes = _allFeeds!
    .where((f) => (f.remainingBags ?? 0) > 0) // فقط دان‌های موجود
    .map((f) => f.name.trim())
    .toSet()
    .toList();
  ...
  void _updateCalculatedWeight(int index) {
    ...
    // final feedsOfType = feeds.where((f) => f.name.trim() == entry.feedType.trim()).toList();
    ...
    for (var feed in feedsOfType) {
      ...
      if (remaining > 0 && bags > 0) {
        ...
        totalBags = (totalBags + remaining).toInt();
      }
    }
