...
class _AddEditIncomeScreenState extends State<AddEditIncomeScreen> {
  ...
  void initState() {
    ...
    _quantityController = TextEditingController(text: _formatNumber(income?.quantity));
    _weightController = TextEditingController(text: _formatNumber(income?.weight));
    _unitPriceController = TextEditingController(text: _formatNumber(income?.unitPrice));
    _descriptionController = TextEditingController(text: income?.description ?? '');
    _selectedDate = income?.date ?? Jalali.now().toDateTime().toIso8601String().substring(0, 10);
  ...
  Future<void> _saveForm() async {
    ...
    if (widget.category == 'متفرقه') {
      final quantity = int.tryParse(_quantityController.text.replaceAll(',', '')) ?? 0;
      final weight = double.tryParse(_weightController.text.replaceAll(',', '')) ?? 0.0;
      if (quantity <= 0 && weight <= 0) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('برای دسته متفرقه، باید حداقل مقدار "تعداد" یا "وزن" را وارد کنید.'), backgroundColor: Colors.red));
        return;
      }
    }
    final income = Income(
  ...
  Widget build(BuildContext context) {
    ...
    final String titleLabel = (widget.category == 'متفرقه') ? 'نام جنس' : 'عنوان';
    return Scaffold(
      appBar: AppBar(
      ...
      body: Form(
        ...
        child: ListView(
          ...
          children: [
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    TextFormField(
                      ...
                      validator: (v) => v!.isEmpty ? '$titleLabel الزامی است' : null,
                    ),
                  ],
                ),
              ),
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      NumericTextFormField(
                        ...
                        validator: (v) => (v == null || v.isEmpty || (double.tryParse(v.replaceAll(',', '')) ?? 0) <= 0) ? 'وزن الزامی است' : null,
                      ),
                    ],
                  ),
                ),
            ...
            // ✅✅✅ ۲. بازطراحی کامل بخش متفرقه با دکمه‌های رادیویی ✅✅✅
            if (widget.category == 'متفرقه') ...[
              Card(
                elevation: 4,
                shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
                ...
                child: Padding(
                  ...
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('ثبت بر اساس', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: primaryColor)),
                      const SizedBox(height: 12),
                      Row(
                        children: [
                          Expanded(
                            child: RadioListTile<MeasureBy>(
                              ...
                              onChanged: (value) => setState(() => _measureBy = value!),
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    NumericTextFormField(
                      ...
                      decoration: InputDecoration(
                        labelText: (widget.category == 'فروش کود' || widget.category == 'فروش مرغ') ? 'قیمت واحد (هر کیلو)' : 'قیمت واحد (تومان)',
                        border: OutlineInputBorder(borderRadius: BorderRadius.circular(8)),
                        enabledBorder: OutlineInputBorder(borderSide: BorderSide(color: primaryColor.withOpacity(0.3))),
                      ),
                    ),
            ...
            ElevatedButton(
              onPressed: _saveForm,
              child: Text(_isEditing ? 'ذخیره تغییرات' : 'ثبت درآمد', style: const TextStyle(fontSize: 16)),
