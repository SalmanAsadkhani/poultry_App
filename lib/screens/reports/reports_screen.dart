...
class _ReportsScreenState extends State<ReportsScreen> with TickerProviderStateMixin {
      ...
      setState(() {
        ...
        if (_searchQuery.isNotEmpty) {
          try {
            ...
            int week = _getWeekNumber(searchAge);
            int tabIndex = _weeks.indexOf(week) + 1;
  ...
  // ✅ [اصلاح شده] محاسبه صحیح سن بر اساس تاریخ گزارش
  ...
  // ✅ [اصلاح شده] محاسبه شماره هفته بر اساس روز
  int _getWeekNumber(int ageInDays) {
    if (ageInDays <= 0) return 1;
    // روز 1-7 → هفته 1
    // روز 8-14 → هفته 2
    // روز 15-21 → هفته 3
    return ((ageInDays - 1) ~/ 7) + 1;
  }
  ...
  List<int> _getWeeks() {
    ...
    final groupedReports = groupBy(validReports, (DailyReport report) {
      ...
      return _getWeekNumber(days);  // ✅ استفاده از متد جدید
    });
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        bottom: TabBar(
          ...
          isScrollable: true,
          labelColor: const Color.fromARGB(255, 2, 43, 66),
          unselectedLabelColor: Colors.white70,
          indicatorColor: const Color.fromARGB(255, 1, 61, 95),
          tabs: [
